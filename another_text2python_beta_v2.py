...
st.markdown("""
Transform affirmations into **subliminal audio fields** with high-speed speech, optional background music, whisper layering, Solfeggio frequencies, ambient soundscapes, and more.
""")
...
with st.form("subliminal_form"):
    ...
    music_file = st.file_uploader("🎼 Upload Custom Background Music (mp3 or wav):", type=["mp3", "wav"])
    bg_sound = st.selectbox("🌌 Choose Ambient Soundscape", ["None", "White Noise", "Pink Noise", "Brown Noise", "Rain", "Ocean Waves", "Wind", "Binaural Theta", "Silence"])
    volume_mix = st.slider("🔊 Background Sound Volume (relative to voice):", 0, 100, 30)
...
if submitted:
    if not text_input.strip():
        ...
        with st.spinner("Generating audio..."):
            ...
            # Load user music or ambient sound
            if music_file:
                ...
                if bg_sound == "White Noise":
                    background = WhiteNoise().to_audio_segment(duration=duration, volume=-20)
                elif bg_sound == "Pink Noise":
                background = WhiteNoise().low_pass_filter(1000).to_audio_segment(duration=duration, volume=-20)
                elif bg_sound == "Brown Noise":
                    background = WhiteNoise().low_pass_filter(400).to_audio_segment(duration=duration, volume=-20)
                elif bg_sound in ["Rain", "Ocean Waves", "Wind", "Binaural Theta"]:
                    ambient_file = os.path.join("assets", "sounds", bg_sound.replace(" ", "_").lower() + ".wav")
                    if os.path.exists(ambient_file):
                        background = AudioSegment.from_file(ambient_file)
                    else:
                        st.warning(f"Ambient file not found: {ambient_file}")

            background = background - (100 - volume_mix)
            combined = background.overlay(voice, loop=True)
            ...
            session_data = {
                ...
                "solfeggio": solfeggio_freq if pro_mode else None,
