...
if TYPE_CHECKING:
    from audio_state_definitions import AudioData, SampleRate, SourceInfo, SourceInfoFrequency, SourceInfoNoise, SourceInfoTTS, SourceInfoUpload
    ...
    try:
        if source_type == "upload":
            ...
            if temp_file_path and os.path.exists(temp_file_path):
                ...
                logger.error(f"load_or_regenerate_audio: Temporary upload file not found for '{original_filename}' at path: {temp_file_path}")
                st.error(f"Source file '{original_filename}' missing. Please remove and re-add the track.", icon="⚠️")
            ...
            if text:
                ...
                if loaded_sr and loaded_sr != sr_hint:
                    logger.warning(f"TTS generated audio at {loaded_sr}Hz, expected {sr_hint}Hz. Resampling may occur later if needed.")
                ...
                logger.error("load_or_regenerate_audio: Missing text in source_info for TTS track.")
                return None
            ...
            if noise_type:
                ...
                logger.error("load_or_regenerate_audio: Missing noise_type for noise track.")
                return None
            ...
            if freq_type:
                ...
                if freq_type == "binaural":
                    ...
                    logger.error(f"load_or_regenerate_audio: Unknown frequency type '{freq_type}'.")
                    return None
                final_sr = sr_hint  # Generation uses the target SR
            else:
                logger.error("load_or_regenerate_audio: Missing freq_type for frequency track.")
                return None

        else:
            logger.error(f"load_or_regenerate_audio: Unknown source_type '{source_type}'.")
            return None

    except Exception as e:
        logger.exception(f"load_or_regenerate_audio: Error processing source_type '{source_type}'")
        st.error(f"Error preparing audio: {e}", icon="🔥")
        return None

    # Final check if audio data was successfully obtained
    if full_audio is None:
        logger.error(f"load_or_regenerate_audio: Failed to get full audio data (Type: {source_type}).")
        return None
    if full_audio.size == 0:
        logger.warning(f"load_or_regenerate_audio: Loaded/generated audio is empty (Type: {source_type}).")
        # Return empty array but with correct SR
        return full_audio.astype(np.float32), final_sr

    logger.info(f"load_or_regenerate_audio: Successfully retrieved full audio (Type: {source_type}, {len(full_audio) / final_sr:.2f}s)")
