...
from config import (
    GLOBAL_SR,
    MIX_PREVIEW_DURATION_S,  # Import needed for mix_tracks default
    MIX_PREVIEW_PROCESSING_BUFFER_S,  # Import needed for mix_tracks default
    PREVIEW_DURATION_S,  # <<<--- ADDED IMPORT
...
def apply_speed_change(audio: AudioData, sr: SampleRate, speed_factor: float) -> AudioData:
    ...
    try:
        ...
        audio_stretched_transposed = librosa.effects.time_stretch(y=audio_contiguous_transposed, rate=speed_factor)
        # Transpose back to (samples, channels)
...
def apply_pitch_shift(audio: AudioData, sr: SampleRate, n_steps: float) -> AudioData:
    ...
    try:
        ...
        if abs(n_steps) > max(12, max_possible_steps):  # Warn for shifts > octave or potentially exceeding Nyquist
            logger.warning(f"Large pitch shift ({n_steps:.1f} steps) requested for SR={sr}Hz. Result quality may be poor or contain artifacts.")

        # librosa expects (channels, samples) and contiguous array
        audio_contiguous_transposed = np.ascontiguousarray(audio.T)
        # Apply pitch shifting
        audio_shifted_transposed = librosa.effects.pitch_shift(y=audio_contiguous_transposed, sr=sr, n_steps=n_steps)
        # Transpose back to (samples, channels)
...
def apply_ultrasonic_shift(audio: AudioData, sr: SampleRate, target_freq: float = ULTRASONIC_TARGET_FREQ) -> AudioData:
    ...
    try:
        ...
        if len(valid_pitches) > 0:
            ...
            logger.warning(f"Could not estimate fundamental frequency, using default {fundamental_freq} Hz.")
        ...
        logger.info(f"Calculated steps for ultrasonic shift: {n_steps:.2f} semitones (Target: {target_freq} Hz, Estimated Fundamental: {fundamental_freq:.1f} Hz)")
        ...
        if normalized_cutoff >= 1.0:
            logger.warning(f"Ultrasonic filter cutoff ({filter_cutoff}Hz) is >= Nyquist frequency ({nyquist}Hz). Skipping filter.")
            return audio_shifted  # Return shifted audio without filtering
        if normalized_cutoff <= 0:
            logger.warning(f"Ultrasonic filter cutoff ({filter_cutoff}Hz) is invalid (<= 0). Skipping filter.")
            return audio_shifted  # Return shifted audio without filtering

        logger.info(f"Applying {filter_order}-order low-pass Butterworth filter at {filter_cutoff} Hz to remove audible artifacts after ultrasonic shift.")
        ...
        if audio_shifted.shape[0] <= min_len_filtfilt:
            logger.warning(f"Shifted audio is too short ({audio_shifted.shape[0]} samples) for the filter length ({min_len_filtfilt}). Skipping filter to avoid errors.")
            return audio_shifted  # Return shifted audio without filtering
...
def apply_standard_filter(audio: AudioData, sr: SampleRate, filter_type: str, cutoff: float) -> AudioData:
    ...
    try:
        ...
        logger.info(f"Applying {filter_order}-order Butterworth {filter_type} filter at {cutoff} Hz.")
        ...
        if audio.shape[0] <= min_len_filtfilt:
            msg = f"Track is too short ({audio.shape[0]} samples) for the filter length ({min_len_filtfilt}). Skipping filter."
            logger.warning(msg)
            # TODO: Remove direct Streamlit call. Raise exception or return error status.
            st.warning(msg)
            return audio
...
def get_preview_audio(
    ...
    preview_duration_s: int = PREVIEW_DURATION_S,  # Use imported constant
) -> Optional[AudioData]:
    """
    ...
    try:
        ...
        if processed_preview.ndim == 2 and processed_preview.shape[1] == 2:
            ...
            logger.warning(f"Processed preview for '{track_name}' has unexpected shape {processed_preview.shape}. Cannot apply volume/pan correctly.")
...
def mix_tracks(
    ...
    preview: bool = False,
    preview_duration_s: int = MIX_PREVIEW_DURATION_S,  # Use imported constant
    preview_buffer_s: int = MIX_PREVIEW_PROCESSING_BUFFER_S,  # Use imported constant
    ...
    for track_id, t_data in tracks_dict.items():
        ...
        if is_active:
            ...
            logger.debug(f"Track '{t_data.get('name', track_id)}': Active. Original len={original_len}, Speed={speed_factor:.2f}, Estimated len={estimated_len}")
        else:
    ...
    for track_id in valid_track_ids_for_mix:
        ...
        if len(mix_buffer) < actual_max_len_samples:
            logger.warning(f"Resizing mix buffer from {len(mix_buffer)} to {actual_max_len_samples} samples due to looping/track length of '{track_name}'.")
