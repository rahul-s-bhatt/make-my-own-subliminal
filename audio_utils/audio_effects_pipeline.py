...
from audio_utils.audio_effects import (
    apply_pitch_shift,
    apply_reverse,
    apply_speed_change,
...
def apply_all_effects(
    track_data: "TrackDataDict", audio_segment: AudioData
) -> AudioData:
    """
    ...
    if audio_segment.size == 0:
        logger.warning(
            f"Input audio segment for effects is empty for '{track_name}'. Returning empty."
        )
    ...
    try:
        ...
        if not np.isclose(speed_factor, 1.0):
            ...
            logger.debug(
                f"'{track_name}': Applied speed change (factor {speed_factor:.2f})."
            )
        ...
        if use_ultrasonic:
            ...
            logger.debug(
                f"'{track_name}': Applying Regular pitch shift ({pitch_shift_steps:.1f} steps)."
            )
        ...
        if not use_ultrasonic and filter_type != "off":
            logger.debug(
                f"'{track_name}': Applying standard filter ({filter_type} @ {filter_cutoff}Hz)."
            )
            audio = apply_standard_filter(audio, sr, filter_type, filter_cutoff)

    except Exception as e:
        logger.exception(
            f"Error during effect application pipeline for track '{track_name}'."
        )
        # Return empty array on pipeline failure
        return np.zeros((0, 2), dtype=np.float32)

    logger.debug(
        f"Finished applying effects for '{track_name}'. Output shape: {audio.shape}"
    )
