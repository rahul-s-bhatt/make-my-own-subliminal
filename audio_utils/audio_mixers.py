...
# Import the new audio loader function
...
# --- ADDED: Import BaseTTSGenerator for type hinting ---
...
# --- MODIFIED: Added tts_generator argument ---
def mix_tracks(
    tracks_dict: Dict[TrackID, TrackDataDict],
    tts_generator: BaseTTSGenerator,  # Added argument
    ...
    # --- Track Selection Logic (Solo/Mute) --- (Keep as is)
    active_track_ids = []
    ...
    # --- Preview Mode --- (No change needed here as it uses snippets)
    if preview:
        ...
        for track_id in active_track_ids:
            ...
            if t_data.get("loop_to_fit", False) and processed_len > 0 and processed_len < target_mix_len_samples:
                ...
                try:
                    ...
                    logger.error(f"Preview: Error concatenating looped preview for '{track_name}': {e_concat}. Using non-looped/padded.")
            ...
            if len(final_preview_segment) > target_mix_len_samples:
                ...
                final_preview_segment = np.pad(final_preview_segment, ((0, target_mix_len_samples - len(final_preview_segment)), (0, 0)), mode="constant")
    ...
    # --- Export Mode ---
        ...
        for track_id in active_track_ids:
            ...
            if not t_data.get("loop_to_fit", False):
                ...
                if loaded_sr != t_data.get("sr", GLOBAL_SR):
                    logger.info(f"Updating SR for track '{track_name}' from {t_data.get('sr')} to {loaded_sr}.")
        ...
        # Determine target length (keep existing logic)
        ...
        if target_mix_len_samples <= 0:
            # Estimate from looping tracks if needed (keep existing logic)
        ...
        # Process looping tracks (keep existing logic, but pass tts_generator)
        ...
        for track_id in active_track_ids:
            ...
            full_audio_tuple = load_or_regenerate_audio(
                ...
                tts_generator=tts_generator,  # Pass the instance
                sr_hint=t_data.get("sr", GLOBAL_SR),
                required_duration_samples=target_mix_len_samples,
            )
            # --- END MODIFIED ---
            if full_audio_tuple is None:
            ...
            if processed_full is None or processed_full.size == 0:
                ...
                continue
            # Loop/pad/truncate logic (keep as is)
    ...
    # --- Final Mixing --- (Keep existing logic)
    ...
    for track_id, processed_audio in processed_tracks.items():
        t_data = tracks_dict[track_id]
        track_name = t_data.get("name", track_id)
        # Adjust length if needed (keep as is)
        ...
        # Apply pan and volume (keep as is)
    ...
    # Final clipping and return (keep as is)
...
# ==========================================
# (mix_wizard_tracks function remains unchanged as it doesn't call load_or_regenerate_audio)
