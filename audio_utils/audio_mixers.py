...
def mix_tracks(
    ...
    if preview:
        ...
        for track_id in active_track_ids:
            ...
            if (
                ...
                logger.debug(
                    f"Preview: Looping processed snippet for '{track_name}' ({processed_len} -> {target_mix_len_samples})"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Preview: Error concatenating looped preview for '{track_name}': {e_concat}. Using non-looped."
                    )
        ...
        for track_id in active_track_ids:
            ...
            if not t_data.get("loop_to_fit", False):
                ...
                full_audio_tuple = load_or_regenerate_audio(
                    t_data.get("source_info"), sr_hint=t_data.get("sr", GLOBAL_SR)
                )
                ...
                if loaded_sr != t_data.get("sr", GLOBAL_SR):
                    logger.info(
                        f"Updating SR for track '{track_name}' from {t_data.get('sr')} to {loaded_sr} based on loaded audio."
                    )
                    t_data["sr"] = (
                        loaded_sr  # Modify dict in place for effects calculation
                    )
        ...
        for track_id in active_track_ids:
            ...
            if len(final_audio_for_track) > target_mix_len_samples:
                ...
                final_audio_for_track = np.pad(
                    ...
                    ((0, target_mix_len_samples - len(final_audio_for_track)), (0, 0)),
                    mode="constant",
                )
    ...
    for track_id, processed_audio in processed_tracks.items():
        ...
        if current_len > final_mix_len:
            ...
            logger.warning(
                f"Padding track '{track_name}' length ({current_len}) to match final mix ({final_mix_len})."
            )
            audio_adjusted = np.pad(
                processed_audio,
                ((0, final_mix_len - current_len), (0, 0)),
                mode="constant",
            )
        else:
        ...
        try:
            if audio_adjusted.ndim == 2 and audio_adjusted.shape[1] == 2:
                ...
                logger.warning(
                    f"Track '{track_name}' is mono during final mixing. Splitting."
                )
                ...
                logger.error(
                    f"Track '{track_name}' has unexpected shape {audio_adjusted.shape}. Cannot add."
                )
        except ValueError as e_add:
            logger.error(
                f"Error adding track '{track_name}' to mix buffer: {e_add}. Buffer: {mix_buffer.shape}, Track: {audio_adjusted.shape}"
            )
    ...
    for track in processed_tracks:
        ...
        if track_len == target_mix_len_samples:
            ...
            if track.shape[1] == 2:
                ...
                logger.warning(
                    f"Wizard Mix: Track shape {track.shape} unexpected. Skip add."
                )
        else:
            logger.error(
                f"Wizard Mix: Track length {track_len} != target {target_mix_len_samples}. Skip add."
            )
    final_mix = np.clip(mix_buffer, -1.0, 1.0)
