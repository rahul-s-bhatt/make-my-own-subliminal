...
def get_preview_audio(
    ...
    logger.info(
        f"Generating preview audio for track '{track_name}' ({track_id}) using snippet (max {preview_duration_s}s)"
    )
    ...
    if audio_snippet is None or audio_snippet.size == 0:
        logger.warning(
            f"No audio snippet data found for track '{track_name}'. Cannot generate preview."
        )
        return None
    if sr <= 0:
        logger.warning(
            f"Invalid sample rate ({sr}) for track '{track_name}'. Cannot generate preview."
        )
        return None
    ...
    try:
        ...
        if processed_preview is None or processed_preview.size == 0:
            logger.warning(
                f"Applying effects resulted in empty audio for '{track_name}' preview."
            )
            return None
        ...
        if (
            ...
            logger.debug(
                f"Looping processed snippet for '{track_name}' preview ({current_processed_len} -> {target_preview_samples})"
            )
            ...
            try:
                ...
                logger.debug(
                    f"Looping complete for preview. New length: {len(processed_preview)}."
                )
            except ValueError as e_concat:
                logger.error(
                    f"Error concatenating looped preview for '{track_name}': {e_concat}. Using non-looped."
                )
        ...
        # Truncate if longer than target
            ...
            logger.debug(
                f"Truncating processed preview for '{track_name}' to {target_preview_samples} samples."
            )
            processed_preview = processed_preview[:target_preview_samples]
        ...
        logger.debug(
            f"Applying Volume ({vol:.2f}) / Pan ({pan:.2f}) to preview for '{track_name}'"
        )
        ...
        if processed_preview.ndim == 2 and processed_preview.shape[1] == 2:
            ...
            logger.warning(
                f"Preview for '{track_name}' is mono after effects. Applying volume, pan ignored."
            )
            processed_preview *= vol
            processed_preview = np.stack([processed_preview, processed_preview], axis=1)
        else:
            logger.warning(
                f"Processed preview for '{track_name}' has unexpected shape {processed_preview.shape}. Cannot apply volume/pan."
            )
            try:
        ...
        logger.debug(
            f"Preview generation complete for '{track_name}'. Final shape: {processed_preview.shape}"
        )
