...
# Version: 5.11 (Fix: Delete Track Re-adds Upload - Attempt 2)
# ==========================================
# --- Early Config ---
import os
...
try:
    ...
    from scipy import signal  # Ensure signal is imported
    ...
    if missing_lib == "docx":
        ...
        install_cmd = "pip install scipy"  # Add scipy if missing
...
ULTRASONIC_TARGET_FREQ = 17500  # Target frequency in Hz for "silent" shift (can be adjusted)
ULTRASONIC_FILTER_CUTOFF = 17000  # Low-pass filter slightly below target to remove artifacts
ULTRASONIC_FILTER_ORDER = 8  # Steep filter order

# --- Resource Limits ---
MAX_AUDIO_DURATION_S = 300  # 5 minutes
MAX_AFFIRMATION_CHARS = 5000  # Limit for TTS input
# -----------------------------

logger.debug(f"Editor Preview duration: {PREVIEW_DURATION_S}s")
logger.debug(f"Mix Preview duration: {MIX_PREVIEW_DURATION_S}s")
logger.debug(f"Ultrasonic Target Frequency: {ULTRASONIC_TARGET_FREQ} Hz")
logger.debug(f"Ultrasonic Filter Cutoff: {ULTRASONIC_FILTER_CUTOFF} Hz (Order: {ULTRASONIC_FILTER_ORDER})")
logger.info(f"Max Upload Duration: {MAX_AUDIO_DURATION_S}s")
logger.info(f"Max Affirmation Chars: {MAX_AFFIRMATION_CHARS}")
...
# ==========================================
# (Functions load_audio, save_audio, generators etc. remain unchanged)
...
def generate_pink_noise(num_samples: int, sr: SampleRate) -> AudioData:
    ...
    b, a = signal.butter(1, 0.01, btype="highpass")
    pink_approx = signal.lfilter(b, a, white, axis=0)
    max_val = np.max(np.abs(pink_approx))
    if max_val > 1e-6:
        pink_approx /= max_val
    return pink_approx.astype(np.float32)
...
def generate_brown_noise(num_samples: int, sr: SampleRate) -> AudioData:
    ...
    max_val = np.max(np.abs(brown_approx))
    if max_val > 1e-6:
        brown_approx /= max_val
...
def apply_pitch_shift(audio: AudioData, sr: SampleRate, n_steps: float) -> AudioData:
    ...
    if not np.isclose(n_steps, 0.0):
        try:
            ...
            audio_contiguous = np.ascontiguousarray(audio.T)
            max_possible_steps = 12 * np.log2((sr / 2) / 440.0)
            if abs(n_steps) > max_possible_steps:
                logger.warning(f"Large pitch shift ({n_steps} steps) may exceed Nyquist limit for SR={sr}. Result quality may be poor.")
...
# --- MODIFIED: Function for Ultrasonic Pitch Shift with Filtering ---
def apply_ultrasonic_shift(audio: AudioData, sr: SampleRate, target_freq: float = ULTRASONIC_TARGET_FREQ) -> AudioData:
    """
    Shifts the audio significantly upwards towards a target frequency,
    then applies a steep low-pass filter to remove audible artifacts.
    WARNING: Still experimental. Quality depends heavily on source and parameters.
    """
    ...
    try:
        ...
        if len(valid_pitches) > 0:
            fundamental_freq = np.median(valid_pitches[valid_pitches > 0])
            ...
            fundamental_freq = 440.0
            logger.warning("Could not estimate fundamental frequency, using default 440 Hz.")

        if fundamental_freq <= 0:
            n_steps = 0
            logger.warning("Fundamental frequency <= 0, cannot calculate ultrasonic shift steps.")
        else:
            n_steps = 12 * np.log2(target_freq / fundamental_freq)
        logger.info(f"Calculated steps for ultrasonic shift: {n_steps:.2f} semitones (Target: {target_freq} Hz)")

        if abs(n_steps) > 36:
            logger.warning(f"Applying very large pitch shift ({n_steps:.1f} steps) for ultrasonic effect. Audio quality might be significantly degraded.")

        audio_contiguous = np.ascontiguousarray(audio.T)
        audio_shifted = librosa.effects.pitch_shift(audio_contiguous, sr=sr, n_steps=n_steps).T.astype(np.float32)
        logger.debug(f"Pitch shift complete. Shape: {audio_shifted.shape}")

        # --- Step 2: Apply Steep Low-Pass Filter ---
        ...
        if normalized_cutoff >= 1.0:
            logger.warning(f"Ultrasonic filter cutoff ({filter_cutoff}Hz) >= Nyquist ({nyquist}Hz). Skipping filter.")
            return audio_shifted
        if normalized_cutoff <= 0:
            logger.warning(f"Ultrasonic filter cutoff ({filter_cutoff}Hz) is invalid. Skipping filter.")
            return audio_shifted

        logger.info(f"Applying {filter_order}-order low-pass filter at {filter_cutoff} Hz to remove ultrasonic artifacts.")
        ...
        if audio_shifted.shape[0] <= min_len_filtfilt:
            logger.warning(f"Shifted audio too short ({audio_shifted.shape[0]} samples) for filter length ({min_len_filtfilt}). Skipping filter.")
            return audio_shifted

        audio_filtered = signal.filtfilt(b, a, audio_shifted, axis=0)
        logger.debug(f"Filtering complete. Shape: {audio_filtered.shape}")

        return audio_filtered.astype(np.float32)

    except Exception as e:
        logger.exception("Error applying ultrasonic shift with filtering.")
        st.error(f"Ultrasonic shift failed: {e}")
        return audio
...
def apply_filter(audio: AudioData, sr: SampleRate, filter_type: str, cutoff: float) -> AudioData:
    """Applies standard lowpass or highpass filter."""
    ...
    try:
        ...
        if normalized_cutoff <= 0:
            ...
            st.warning(msg)
            return audio
        filter_order = 4  # Standard order for regular filters
        ...
        logging.exception("Error applying standard filter.")
        st.error(f"Filter failed: {e}")
        return audio


# --- MODIFIED: apply_all_effects to include ultrasonic option ---
def apply_all_effects(track_data: TrackData, audio_segment: Optional[AudioData] = None) -> AudioData:
    """Applies effects sequentially: Reverse -> Speed -> Pitch/Ultrasonic -> Filter."""
    track_name = track_data.get("name", "Unnamed")
    should_reverse = track_data.get("reverse_audio", False)
    use_ultrasonic = track_data.get("ultrasonic_shift", False)  # Get new state
    pitch_shift_steps = track_data.get("pitch_shift", 0)

    if audio_segment is not None:
        audio = audio_segment.copy()
        logger.debug(f"Applying effects to segment for: '{track_name}' (Reverse={should_reverse}, Ultrasonic={use_ultrasonic})")
    else:
        logger.info(f"Applying effects to full original_audio for: '{track_name}' (Reverse={should_reverse}, Ultrasonic={use_ultrasonic})")
    ...
    # 3. Pitch Shift (Ultrasonic OR Regular)
    if use_ultrasonic:
        logger.debug(f"Applying Ultrasonic shift for '{track_name}'")
        audio = apply_ultrasonic_shift(audio, sr, target_freq=ULTRASONIC_TARGET_FREQ)  # Calls updated function
    elif not np.isclose(pitch_shift_steps, 0.0):
        logger.debug(f"Applying Regular pitch shift ({pitch_shift_steps} steps) for '{track_name}'")
        audio = apply_pitch_shift(audio, sr, pitch_shift_steps)
    else:
        logger.debug(f"No pitch shift applied for '{track_name}'")

    # 4. Filter (Standard Low/High Pass)
    if not use_ultrasonic:  # Only apply standard filter if ultrasonic wasn't used
        audio = apply_filter(audio, sr, track_data.get("filter_type", "off"), track_data.get("filter_cutoff", 8000.0))
    else:
        logger.debug(f"Skipping standard filter for '{track_name}' because ultrasonic shift was applied.")
...
def get_preview_audio(track_data: TrackData, preview_duration_s: int = PREVIEW_DURATION_S) -> Optional[AudioData]:
    ...
    try:
        ...
        processed_preview = apply_all_effects(track_data, audio_segment=preview_segment)  # Uses updated apply_all_effects
...
def mix_tracks(tracks_dict: Dict[TrackID, TrackData], preview: bool = False) -> Tuple[Optional[AudioData], Optional[int]]:
    ...
    if preview:
        for track_id in valid_track_ids_for_mix:
            ...
            if original_audio is not None and original_audio.size > 0:
                ...
                processed_segments[track_id] = apply_all_effects(t_data, audio_segment=segment)  # Uses updated apply_all_effects
            else:
    ...
    for track_id in valid_track_ids_for_mix:
        ...
        if preview:
            ...
            if original_audio is None or original_audio.size == 0:
                ...
                continue
            processed_audio = apply_all_effects(t_data)  # Uses updated apply_all_effects
    ...
    return final_mix.astype(np.float32), len(final_mix)
...
class AppState:
    ...
    STATE_KEY = "tracks_non_destructive_v4"
    ...
    def get_default_track_params() -> TrackData:
        ...
        return {
            ...
            "loop_to_fit": False,
            "reverse_audio": False,
            "ultrasonic_shift": False,  # --- NEW STATE for Silent Subliminal ---
    ...
    def add_track(self, track_id: TrackID, track_data: TrackData, track_type: TrackType = TRACK_TYPE_OTHER):
        ...
        if "original_audio" not in track_data or track_data["original_audio"] is None:
            if track_data.get("source_type") != "upload":
                logger.error(f"Attempted add non-upload track {track_id} without original_audio.")
            else:
                logger.info(f"Adding upload track {track_id} without initial audio data (needs re-upload).")
                final_track_data["original_audio"] = None

        gen_keys = [k for k in default_params.keys() if k.startswith("gen_") or k in ["source_type", "original_filename", "tts_text"]]
        for key in gen_keys:
            if key in track_data:
                final_track_data[key] = track_data[key]
        ...
        logger.info(f"Added track ID: {track_id}, Name: '{final_track_data.get('name', 'N/A')}', Type: {track_type}, Source: {final_track_data.get('source_type')}")
    ...
    def update_track_param(self, track_id: TrackID, param_name: str, value: Any):
        ...
        tracks = self._get_tracks_dict()
        if track_id in tracks:
            preview_params = ["volume", "speed_factor", "pitch_shift", "pan", "filter_type", "filter_cutoff", "reverse_audio", "ultrasonic_shift"]
            if (param_name in preview_params and tracks[track_id].get(param_name) != value) or param_name == "original_audio":
            ...
            if param_name in AppState.get_default_track_params():
                ...
                if param_name == "ultrasonic_shift" and value is True:
                    logger.debug(f"Ultrasonic shift enabled for {track_id}, disabling regular pitch shift.")
                    st.session_state[self.STATE_KEY][track_id]["pitch_shift"] = 0
                elif param_name == "pitch_shift" and not np.isclose(value, 0.0):
                    logger.debug(f"Regular pitch shift enabled for {track_id}, disabling ultrasonic shift.")
...
# ==========================================
# (TTSGenerator class remains unchanged)
class TTSGenerator:
...
class UIManager:
    ...
    def _calculate_preview_hash(self, track_data: TrackData) -> int:
        ...
        params_to_hash = (
            ...
            track_data.get("ultrasonic_shift", False),  # Include new state in hash
    ...
    def render_sidebar(self):
        with st.sidebar:
            ...
            st.divider()
            self._render_frequency_generators()  # Modified in Step 2
            st.divider()
            self._render_background_generators()
            st.markdown("---")
            st.subheader("💾 Project")
            self._render_save_load()  # Modified in Step 3 (Error Fix)
            st.markdown("---")
    ...
    def _render_uploader(self):
        ...
        st.caption(f"Upload music, voice recordings, etc. (Max duration: {MAX_AUDIO_DURATION_S // 60} minutes)")  # Show limit
        ...
        if uploaded_files:
            # Get a snapshot of current track filenames BEFORE processing uploads
            current_track_filenames = {tdata.get("original_filename") for tdata in self.app_state.get_all_tracks().values() if tdata.get("source_type") == "upload"}

            for file in uploaded_files:
                # Check if a track with this filename *currently* exists in the state
                if file.name in current_track_filenames:
                    # Check if it has audio data (might be from project load)
                    track_exists_with_audio = False
                    for tid, tdata in self.app_state.get_all_tracks().items():
                        if tdata.get("original_filename") == file.name and tdata.get("original_audio") is not None:
                            track_exists_with_audio = True
                            break
                    if track_exists_with_audio:
                        logger.info(f"Skipping upload: Track for '{file.name}' already exists with audio data.")
                        continue
                    else:
                        # Track exists but needs audio (likely from project load)
                        logger.info(f"File uploader has file '{file.name}' matching existing track without audio. Proceeding to load.")
                        existing_track_id = tid  # Get ID from the loop above
                        existing_track = self.app_state.get_track(existing_track_id)
                else:
                    # File is genuinely new or was deleted in a previous run
                    existing_track = None
                ...
                if audio.size > 0:
                    ...
                    if duration_seconds > MAX_AUDIO_DURATION_S:
                        logger.warning(f"Upload '{file.name}' rejected: Duration {duration_seconds:.1f}s exceeds limit {MAX_AUDIO_DURATION_S}s.")
                        st.error(f"❌ File '{file.name}' is too long ({duration_seconds:.1f}s). Maximum allowed duration is {MAX_AUDIO_DURATION_S // 60} minutes.")
                        continue  # Skip this file
                    # --- End Duration Check ---

                    if existing_track:  # Update track that was missing audio
                        logger.info(f"Updating existing track '{existing_track.get('name')}' ({existing_track_id}) with uploaded audio data for {file.name}.")
                        ...
                        st.success(f"Re-loaded audio for '{existing_track.get('name')}'")
                        st.rerun()  # Rerun needed to show waveform etc.
                    else:  # Add as new track
                        track_id = str(uuid.uuid4())
                        track_params = AppState.get_default_track_params()
                        if "voice" in file.name.lower() or "record" in file.name.lower() or "affirmation" in file.name.lower():
                            track_type = TRACK_TYPE_VOICE
                        elif "music" in file.name.lower() or "background" in file.name.lower() or "mask" in file.name.lower():
                        ...
                        st.success(f"Loaded '{file.name}' as '{track_type}' ({duration_seconds:.1f}s)")  # Show duration
                        # Add filename to our set for this run to prevent immediate re-add if deleted later in same run
    ...
    def _render_affirmation_inputs(self):
        ...
        st.caption(f"Uses system default TTS voice. Max {MAX_AFFIRMATION_CHARS} characters.")  # Show limit
        ...
        with tab1:
            ...
            affirmation_text = st.text_area(
                ...
                height=150,
                key="affirmation_text_area",
                label_visibility="collapsed",
                help="Enter each affirmation on a new line.",
                max_chars=MAX_AFFIRMATION_CHARS,  # Add built-in max_chars
            )
            ...
            if st.button(
                ...
                # Check length again just in case (though max_chars should handle it)
                    ...
                    st.error(f"❌ Text is too long ({len(affirmation_text)} chars). Maximum allowed is {MAX_AFFIRMATION_CHARS}.")
                else:
                    default_name = "TTS Affirmations"
                    self._generate_tts_track(affirmation_text, default_name)
            ...
            if st.button(
                ...
                if uploaded_file:
                    ...
                    if text is not None:
                        ...
                        # --- Check Character Limit for File ---
                        elif len(text) > MAX_AFFIRMATION_CHARS:
                            logger.warning(f"TTS File input '{uploaded_file.name}' rejected: Length {len(text)} exceeds limit {MAX_AFFIRMATION_CHARS}.")
                            st.error(f"❌ Text in file '{uploaded_file.name}' is too long ({len(text)} chars). Maximum allowed is {MAX_AFFIRMATION_CHARS}.")
                        # --- End Character Limit Check ---
                        else:
                            default_name = f"Affirmations ({uploaded_file.name})"
                            self._generate_tts_track(text, default_name)
                    else:
    ...
    def _generate_tts_track(self, text_content: str, track_name: str):
        ...
        # Length check should happen *before* calling this function now
        logger.info(f"Generating TTS track with default name: {track_name}")
        ...
        if audio is not None and sr is not None:
            ...
            self.app_state.add_track(track_id, track_params, track_type=TRACK_TYPE_AFFIRMATION)
            st.success(f"'{track_name}' track generated!")
            st.toast("Affirmation track added!", icon="✅")

    def _render_frequency_generators(self):
        """Renders options for generating frequencies/tones based on app mode."""
        ...
        if "freq_gen_type" in st.session_state and st.session_state.freq_gen_type in available_gen_types:
            default_gen_type = st.session_state.freq_gen_type
        try:
            default_index = available_gen_types.index(default_gen_type)
        except ValueError:
            default_index = 0

        gen_type = st.radio("Select Type:", available_gen_types, index=default_index, key="freq_gen_type", horizontal=True, label_visibility="collapsed")
            ...
            if st.button(f"Generate '{preset_name}' Track", key="generate_preset_freq"):
                with st.spinner("Generating preset frequency..."):
                    ...
                    if audio is not None:
                        ...
                        st.success(f"'{default_track_name}' track generated!")
                    else:
        ...
        if app_mode == "Advanced":
            if gen_type == "Binaural Beats":
                ...
                if st.button("Generate Binaural Track", key="generate_bb", help="Create track with these settings."):
                    ...
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    default_name = f"Binaural {bb_fleft}Hz L / {bb_fright}Hz R"
                    track_params.update(
                        {
                            "original_audio": audio,
                            "sr": GLOBAL_SR,
                    ...
                    self.app_state.add_track(track_id, track_params, track_type=TRACK_TYPE_FREQUENCY)
                    st.success(f"'{default_name}' generated!")
                ...
                if st.button("Generate Isochronic Track", key="generate_iso", help="Create track with these settings."):
                    ...
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    default_name = f"Isochronic {iso_carrier}Hz / {iso_pulse}Hz Pulse"
                    track_params.update(
                        {
                            "original_audio": audio,
                            "sr": GLOBAL_SR,
        ...
        if st.button(f"Generate {noise_type} Track", key="generate_noise"):
            with st.spinner(f"Generating {noise_type}..."):
                ...
                if audio is not None:
                    ...
                    st.success(f"'{default_name}' track generated!")
                else:
    ...
    def _render_save_load(self):
            ...
            try:
                ...
                for track_id, track_data in tracks.items():
                    ...
                    if "preview_temp_file_path" in save_data:
                        del save_data["preview_temp_file_path"]
                    if "preview_settings_hash" in save_data:
                        del save_data["preview_settings_hash"]
        ...
        if uploaded_project_file is not None:
            ...
            st.rerun()  # Rerun is sufficient to trigger loading logic

    def render_tracks_editor(self):
        """Renders the main editor area with all tracks using previews."""
        st.header("🎚️ Tracks Editor")
        tracks = self.app_state.get_all_tracks()
        app_mode = st.session_state.get("app_mode", "Easy")  # Get current mode
        ...
        if not tracks:
            if "welcome_message_shown" in st.session_state:
                ...
                with col_icon:
                    ...
                    st.image(os.path.join("assets", "favico.png") if os.path.exists(os.path.join("assets", "favico.png")) else "🧠", width=80)
                with col_text:
                    st.subheader("✨ Let's Create Your Subliminal!")
                    st.markdown(f"Your project is empty. Use the **sidebar on the left** (👈) to add your first audio layer.")
                    ...
                    if app_mode == "Easy":
                        ...
                        st.markdown("- Add specific **Frequencies/Tones** (Binaural, Solfeggio, Isochronic) or use Presets.")
                st.markdown("---")
                st.info("Once you add a track, the editor controls will appear here.")
            return  # Stop rendering if no tracks

        st.caption(f"Current Mode: **{app_mode}**")
        ...
        for track_id, track_data in list(tracks.items()):
            if track_id not in self.app_state.get_all_tracks():
                continue
            track_name = track_data.get("name", "Unnamed")
            track_type_icon = track_data.get("track_type", TRACK_TYPE_OTHER).split(" ")[0]
            ...
            with st.expander(expander_label, expanded=True):
                ...
                self._render_track_main_col(track_id, track_data, col_main)  # Modified in Step 3 & 5
        ...
        if track_ids_to_delete:
            ...
            if deleted_count > 0:
                ...
                # --- FIX: Restore st.rerun() for immediate UI update ---
                st.rerun()  # ADDED BACK - Let Streamlit handle redraw implicitly
                # --------------------------------------------------------
        # --- End Fix ---

    def _render_track_main_col(self, track_id: TrackID, track_data: TrackData, column: st.delta_generator.DeltaGenerator):
        """Renders the waveform preview and settings controls based on app mode."""
        with column:
            try:
                original_audio = track_data.get("original_audio")
                ...
                if original_audio is not None and original_audio.size > 0:
                    ...
                    elif not preview_displayed_from_cache:
                ...
                with basic_cols[0]:
                    vol = st.slider("Volume", 0.0, 2.0, track_data.get("volume", 1.0), 0.05, key=f"vol_{track_id}", help="Adjust loudness.")
                    if vol != track_data.get("volume"):
                        self.app_state.update_track_param(track_id, "volume", vol)
                with basic_cols[1]:
                    pan = st.slider("Pan", -1.0, 1.0, track_data.get("pan", 0.0), 0.1, key=f"pan_{track_id}", help="Adjust L/R balance.")
                    if pan != track_data.get("pan"):
                        self.app_state.update_track_param(track_id, "pan", pan)

                # Subliminalize Preset Button (Keep visible in both modes for convenience)
                if track_data.get("track_type") == TRACK_TYPE_AFFIRMATION:
                    if st.button(
                        ...
                        st.toast("Speed set to 10x, Volume to 0.05. Click 'Update Preview'.", icon="⚡")  # Added toast
                        st.rerun()

                # --- Advanced Settings (Only in Advanced Mode) ---
                if app_mode == "Advanced":
                    st.markdown("**Advanced Effects**")
                    st.caption("Adjust speed, pitch, filter, loop, reverse. Click 'Update Preview' after changes.")

                    # --- Add Ultrasonic Shift Checkbox ---
                    ultrasonic_value = st.checkbox(
                        "🔊 Ultrasonic Shift ('Silent')",
                        key=f"ultrasonic_{track_id}",
                        value=track_data.get("ultrasonic_shift", False),
                        help=f"EXPERIMENTAL: Shifts audio towards {ULTRASONIC_TARGET_FREQ}Hz and filters artifacts. Disables regular Pitch Shift. Quality may vary.",  # Updated help text
                    )
                    if ultrasonic_value != track_data.get("ultrasonic_shift"):
                        self.app_state.update_track_param(track_id, "ultrasonic_shift", ultrasonic_value)
                        # Rerun needed to disable/enable pitch slider below
                        st.rerun()
                    # --- End Add Checkbox ---
                    ...
                    with col_fx1_1:
                        ...
                        loop_value = st.checkbox(
                            ...
                            help="Loop track to fit project during final mix/export? (Preview not shown)",
                        )
                    ...
                    with col_fx1_3:
                        # --- Disable Pitch slider if Ultrasonic is ON ---
                        ...
                        pitch = st.slider(
                            ...
                            help="Adjust pitch without changing speed. Disabled if Ultrasonic Shift is active.",
                            disabled=pitch_disabled,
                        )
                        if not pitch_disabled and pitch != track_data.get("pitch_shift"):
                            self.app_state.update_track_param(track_id, "pitch_shift", pitch)
                        # --- End Disable Logic ---

                    col_fx2_1, col_fx2_2 = st.columns(2)  # Filter Type, Filter Cutoff
                    with col_fx2_1:
                        f_type = st.selectbox(
                            "Filter",
                            ...
                            help="Apply standard low/high pass filter. (Not applied if Ultrasonic Shift is active).",
                            disabled=track_data.get("ultrasonic_shift", False),  # Disable if ultrasonic is on
                        )
                        if f_type != track_data.get("filter_type"):
                            self.app_state.update_track_param(track_id, "filter_type", f_type)
                    with col_fx2_2:
                        f_enabled = track_data.get("filter_type", "off") != "off" and not track_data.get("ultrasonic_shift", False)  # Also check ultrasonic
                        max_cutoff = (track_sr / 2 - 1) if track_sr else 20000.0
                        f_cutoff = st.number_input(
                            ...
                            help="Filter cutoff frequency. (Not applied if Ultrasonic Shift is active).",
                        )
                        if f_cutoff != track_data.get("filter_cutoff"):
                            self.app_state.update_track_param(track_id, "filter_cutoff", f_cutoff)

                # --- Update Preview Button (Always Visible) ---
        ...
        with column:
            try:
                ...
                if st.button("🗑️ Delete Track", key=f"delete_{track_id}", help="Permanently delete."):
                    delete_clicked = True
                    # The warning below might not be visible if rerun happens immediately
    ...
    def render_master_controls(self):
        """Renders the master preview, export controls."""
        st.divider()
        ...
        with master_cols[1]:
            ...
            if "export_buffer" in st.session_state and st.session_state.export_buffer:
                ...
                download_filename = f"{st.session_state.get('export_filename_sanitized', 'mindmorph_mix')}.{file_ext}"
    ...
    def _handle_preview_click(self):
        ...
        with st.spinner("Generating preview mix..."):
            mix_preview, _ = mix_tracks(tracks, preview=True)
    ...
    def _handle_export_click(self):
        ...
        with st.spinner(f"Generating full mix ({export_format.upper()})... This may take time."):
            full_mix, final_mix_len_samples = mix_tracks(tracks, preview=False)
        ...
        with st.expander("📖 Show Instructions & Notes", expanded=not tracks_exist):
            st.markdown("""
              ...
              **Workflow:**
              ...
              1.  **✨ Select Mode (Top):** Choose 'Easy' (simplified) or 'Advanced' (all features).
              2.  **➕ Add Tracks (Sidebar):** Use the options on the left (options vary by mode).
              3.  **🎚️ Edit Tracks (Main Panel):** Adjust settings (options vary by mode). Click `⚙️ Update Preview` to see changes.
              4.  **🔊 Mix & Export (Bottom Panel):** Preview the combined audio or export the final file.

              **Tips:**
              * Use low volume and high speed (e.g., 2x-10x) for subliminal affirmation tracks. Try the '⚡ Subliminalize Preset' button!
              * Try the '🔊 Ultrasonic Shift' (Advanced Mode) for silent subliminals (experimental).
              * Use the `🔁 Loop` option (Advanced Mode) for short sounds.
              * Save your project configuration using the sidebar option.
              """)  # Added Ultrasonic tip


# ==========================================
# 5. Benchmarking (Placeholder Class)
# ==========================================
# (Benchmarker class remains unchanged)
class Benchmarker:
...
# (show_welcome_message remains unchanged from v5.4)
def show_welcome_message():
    """Displays the welcome message container, explaining modes."""
    if "welcome_message_shown" not in st.session_state:
        with st.container(border=True):
            ...
            button_cols = st.columns([1, 1.5, 1])
            with button_cols[1]:
...
def main():
                ...
                if "tracks" in project_content:
                    ...
                    with st.spinner("Reconstructing project..."):
                        for track_id, track_data in loaded_tracks_data.items():
                            ...
                            # (Audio reconstruction logic remains the same)
                            if source_type == "tts" and "tts_text" in track_data:
                                ...
                                logger.warning(f"Track {track_id} is an upload ('{track_data.get('original_filename')}'). Audio data needs re-upload.")
                            else:
                            ...
                            # Ensure loaded tracks have the new default param
                            if "ultrasonic_shift" not in track_data:
                                track_data["ultrasonic_shift"] = False
                            app_state_temp.add_track(new_track_id, track_data, track_type=track_type)
                    st.success("Project loaded successfully! Re-upload any required audio files.")
                else:
    ...
    if "welcome_message_shown" in st.session_state:
        ...
        try:
            ...
            logger.warning(f"Invalid mode '{st.session_state.app_mode}' found in state. Defaulting to Easy.")
            st.session_state.app_mode = "Easy"
        ...
        ui_manager.render_tracks_editor()  # Contains updated empty state message
