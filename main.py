...
# Version: 5.5 (UX Refinements 1: Default Names & Toast)
# ==========================================
# --- Early Config ---
import os
...
# ==========================================
# (Functions load_audio, save_audio, etc. remain unchanged)
...
# ==========================================
# (AppState class remains unchanged)
class AppState:
    ...
    def add_track(self, track_id: TrackID, track_data: TrackData, track_type: TrackType = TRACK_TYPE_OTHER):
        ...
        # Use provided name if exists, otherwise use default
...
class UIManager:
        ...
        if uploaded_files:
            for file in uploaded_files:
                ...
                for tid, tdata in self.app_state.get_all_tracks().items():
                    if tdata.get("source_type") == "upload" and tdata.get("original_filename") == file.name:
                        existing_track = tdata
                        existing_track_id = tid
                        break

                if existing_track and existing_track.get("original_audio") is not None:
                ...
                if audio.size > 0:
                    if existing_track:
                        ...
                        # --- UX Change: Default track type based on name ---
                        if "voice" in file.name.lower() or "record" in file.name.lower() or "affirmation" in file.name.lower():
                            ...
                            track_type = TRACK_TYPE_OTHER  # Default if unsure
                        # --- UX Change: Use filename as default name ---
                        track_params.update(
                            {
                        ...
                        st.success(f"Loaded '{file.name}' as '{track_type}'")  # Show type
                else:
    ...
    def _render_affirmation_inputs(self):
        ...
        with tab1:
            ...
            if st.button(
                ...
                if affirmation_text:
                    # --- UX Change: Default name for TTS from text ---
        ...
        with tab2:
            ...
            if st.button(
                ...
                if uploaded_file:
                    ...
                    if text is not None:
                        if text.strip():
                            # --- UX Change: Default name for TTS from file ---
    ...
    def _generate_tts_track(self, text_content: str, track_name: str):
        ...
        # --- UX Change: Use provided track_name as default ---
            ...
            track_params.update(
                {
                    "original_audio": audio,
                    "sr": sr,
                    "name": track_name,  # Use the passed default name
                    "source_type": "tts",
        ...
        if gen_type == "Presets":
            ...
            if st.button(f"Generate '{preset_name}' Track", key="generate_preset_freq"):
                with st.spinner("Generating preset frequency..."):
                    audio = None
                    gen_params = {}
                    # --- UX Change: Use preset_name as default name ---
                    ...
                    if audio is not None:
                        ...
                        track_params.update(gen_params)  # Use default name
        ...
        if app_mode == "Advanced":
            if gen_type == "Binaural Beats":
                ...
                if st.button("Generate Binaural Track", key="generate_bb", help="Create track with these settings."):
                    ...
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    # --- UX Change: Default name for Binaural ---
                ...
                if st.button("Generate Solfeggio Track", key="generate_solf", help="Create track with this tone."):
                    ...
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    # --- UX Change: Default name for Solfeggio ---
                ...
                if st.button("Generate Isochronic Track", key="generate_iso", help="Create track with these settings."):
                    ...
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    # --- UX Change: Default name for Isochronic ---
        ...
        if st.button(f"Generate {noise_type} Track", key="generate_noise"):
            with st.spinner(f"Generating {noise_type}..."):
                ...
                if audio is not None:
                    track_id = str(uuid.uuid4())
                    track_params = AppState.get_default_track_params()
                    # --- UX Change: Default name for Noise ---
                    default_name = f"{noise_type}"  # Use the selected noise type
                    track_params.update(
                        {
                            "original_audio": audio,
                            "sr": GLOBAL_SR,
    ...
    def render_tracks_editor(self):
        ...
        # --- START ONBOARDING CHANGE 2: Enhanced Empty State ---
        if not tracks:
            if "welcome_message_shown" in st.session_state:
                ...
                with col_icon:
                    ...
                    st.image(os.path.join("assets", "favico.png") if os.path.exists(os.path.join("assets", "favico.png")) else "🧠", width=80)  # Use icon/logo
                with col_text:
        ...
        # --- END ONBOARDING CHANGE 2 ---

        st.caption(f"Current Mode: **{app_mode}**")  # Show current mode above tracks
        ...
        for track_id, track_data in list(tracks.items()):
            ...
            with st.expander(expander_label, expanded=True):
                ...
                self._render_track_main_col(track_id, track_data, col_main)  # Modified in Step 3
            ...
            try:
                ...
                if track_data.get("track_type") == TRACK_TYPE_AFFIRMATION:
                    if st.button(
                        ...
                        # --- UX Change: Add toast message ---
                        st.toast("Speed set to 10x, Volume to 0.05. Click 'Update Preview'.", icon="⚡")
                        # --- End UX Change ---
                        st.rerun()  # Rerun to update sliders if they are visible (Advanced mode)
                ...
                if app_mode == "Advanced":
                    ...
                    with col_fx2_2:
                        f_enabled = track_data.get("filter_type", "off") != "off"  # Use .get() for safety
                        max_cutoff = (track_sr / 2 - 1) if track_sr else 20000.0  # Handle potential zero SR
...
# --- START ONBOARDING CHANGE 1: Updated Welcome Message ---
...
def main():
    ...
    # --- Render main UI only if welcome message is dismissed ---
    if "welcome_message_shown" in st.session_state:
        # --- Mode Switcher (Moved here to be always visible after welcome) ---
