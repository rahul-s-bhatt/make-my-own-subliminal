        ...
        with column:
            try:
                ...
                try:
                    ...
                    current_index = TRACK_TYPES.index(
                        TRACK_TYPE_OTHER
                    )  # Default if type is somehow invalid
                new_type = st.selectbox(
                    "Track Type",
                    TRACK_TYPES,
                    index=current_index,
                    key=f"type_{track_id}",
                    help="Categorize this layer.",
                )
                ...
                with ms_col1:
                    ...
                    mute = st.checkbox(
                        "Mute",
                        value=mute_value,
                        key=f"mute_{track_id}",
                        help="Silence track in mix.",
                    )
                    if mute != mute_value:
                ...
                with ms_col2:
                    ...
                    solo = st.checkbox(
                        "Solo",
                        value=solo_value,
                        key=f"solo_{track_id}",
                        help="Isolate track(s) in mix.",
                    )
                    if solo != solo_value:
                ...
                if st.button(
                    "🗑️ Delete Track",
                    key=f"delete_{track_id}",
                    help="Permanently remove track.",
                    type="secondary",
                    use_container_width=True,
                ):
                    delete_clicked = True
                    logger.info(
                        f"Delete button clicked for track {track_id} ('{track_data.get('name', 'N/A')}')"
                    )

            except Exception as e:
                logger.exception(
                    f"Error rendering metadata column for track {track_id}"
                )
