...
class TTSGenerator:
    ...
    def __init__(self, chunk_size: int = TTS_CHUNK_SIZE):
        ...
        logger.debug(
            f"TTSGenerator initialized with chunk_size={chunk_size}, rate={self.rate}, volume={self.volume}"
        )
    ...
    def _synthesize_chunk(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                delete=False, suffix=f"_chunk{chunk_index}.wav", mode="wb"
            ) as tmp:
                temp_chunk_path = tmp.name
            logger.debug(
                f"Synthesizing chunk {chunk_index}/{total_chunks} to temporary file: {temp_chunk_path}..."
            )
            ...
            if (
                not os.path.exists(temp_chunk_path)
                or os.path.getsize(temp_chunk_path) == 0
            ):
                logger.error(
                    f"TTS engine failed to create a non-empty file for chunk {chunk_index}: {temp_chunk_path}"
                )
                # TODO: Remove direct Streamlit call. Return status/raise exception.
                st.error(
                    f"TTS engine failed for chunk {chunk_index}. Check system TTS setup."
                )
            ...
            if temp_chunk_path and os.path.exists(temp_chunk_path):
                try:
                    ...
                    logger.warning(
                        f"Could not clean up failed chunk file: {temp_chunk_path}"
                    )
            return None

    def _load_and_resample_chunk(
        ...
        try:
            ...
            if audio_data.shape[1] == 1:
                ...
                logger.warning(
                    f"Chunk {file_path} has unexpected channel count: {audio_data.shape[1]}. Attempting to use first two."
                )
            ...
            if sr != target_sr:
                logger.warning(
                    f"Sample rate mismatch in chunk {file_path}! Expected {target_sr}, got {sr}. Resampling..."
                )
                if audio_data.size > 0:
                    # Librosa expects (channels, samples) for resample
                    audio_data_resampled = librosa.resample(
                        audio_data.T, orig_sr=sr, target_sr=target_sr
                    )
            ...
            logger.debug(
                f"Loaded chunk {file_path}. Shape: {audio_data.shape}, SR: {target_sr}"
            )
        ...
        try:
            ...
            with st.spinner(f"Synthesizing {num_chunks} audio chunks..."):
                ...
                for i, chunk_text in enumerate(chunks):
                    ...
                    status_text.text(
                        f"Synthesizing audio chunk {chunk_index}/{num_chunks}..."
                    )
                    progress_placeholder.progress(i / num_chunks)

                    chunk_start_time = time.time()
                    temp_file = self._synthesize_chunk(
                        chunk_text, chunk_index, num_chunks
                    )
                    ...
                    if temp_file:
                        ...
                        logger.debug(
                            f"Chunk {chunk_index} synthesized in {chunk_end_time - chunk_start_time:.2f}s."
                        )
                    else:
                        # Synthesis failed for this chunk, stop processing
                        logger.error(
                            f"Synthesis failed for chunk {chunk_index}. Aborting TTS generation."
                        )
                        # No need to return here, finally block will clean up
                        raise RuntimeError(
                            f"TTS synthesis failed for chunk {chunk_index}"
                        )

                synthesis_end_time = time.time()
                logger.info(
                    f"Finished synthesizing all {num_chunks} chunks in {synthesis_end_time - synthesis_start_time:.2f}s."
                )
            ...
            logger.info(
                f"Loading and combining {len(temp_chunk_files)} synthesized audio chunks."
            )
            for i, file_path in enumerate(temp_chunk_files):
                # TODO: Update progress reporting without direct st calls
                status_text.text(f"Loading chunk {i + 1}/{len(temp_chunk_files)}...")
                audio_data, chunk_sr = self._load_and_resample_chunk(
                    file_path, GLOBAL_SR
                )
                ...
                if audio_data is not None and chunk_sr is not None:
                    ...
                    logger.error(
                        f"Failed to load or process chunk file: {file_path}. Aborting."
                    )
            ...
            if final_sr is None:
                ...
                logger.error(
                    "Could not determine the final sample rate after combining chunks."
                )
                # TODO: Remove direct Streamlit call. Raise exception.
            ...
            if final_sr != GLOBAL_SR:
                logger.warning(
                    f"Final audio SR ({final_sr}) unexpectedly differs from target ({GLOBAL_SR}). This indicates a potential issue in resampling."
                )
                # Attempt resampling again as a fallback? Or just log the warning.
                # For now, just log it. The audio might still be usable.

            logger.info(
                f"TTS generation complete. Final audio shape: {final_audio.shape}, SR: {final_sr}Hz"
            )
            ...
            if temp_chunk_files:
                logger.debug(
                    f"Cleaning up {len(temp_chunk_files)} temporary TTS chunk files..."
                )
                cleaned_count = 0
                for file_path in temp_chunk_files:
                    if file_path and os.path.exists(file_path):
                        try:
                            ...
                            logger.warning(
                                f"Could not delete temporary TTS chunk file {file_path}: {e_os}"
                            )
