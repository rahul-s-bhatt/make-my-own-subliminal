...
def setup_logging():
    ...
    if "log_listener_started" not in st.session_state:
        try:
            ...
            logger.info(
                "Logging QueueListener started with FileHandler and StreamHandler."
            )
        except Exception as e:
    ...
    try:
        ...
        if file_name.lower().endswith(".txt") or file_type == "text/plain":
            ...
            try:
                ...
                st.error(
                    f"Error decoding text file '{file_name}': {e_decode}. Ensure it's UTF-8 encoded."
                )
                return None
        ...
        # --- Read .docx file ---
            ...
            if not DOCX_AVAILABLE:
                logger.error(
                    "Attempted to read .docx file, but 'python-docx' library is not installed."
                )
                st.error(
                    "Cannot read .docx files. Please install the 'python-docx' library (`pip install python-docx`)."
                )
                return None
            ...
            try:
                ...
                logger.debug(
                    f"docx document parsed. Found {len(document.paragraphs)} paragraphs."
                )
                ...
                for i, para in enumerate(document.paragraphs):
                    if para is None:
                        logger.warning(
                            f"Paragraph {i} in '{file_name}' is None. Skipping."
                        )
                        continue
                ...
                logger.debug(
                    f"Extracted {len(para_texts)} non-empty paragraphs from '{file_name}'."
                )
        ...
        # --- Unsupported file type ---
            ...
            logger.error(
                f"Unsupported file type for text reading: {file_type} (Filename: {file_name})"
            )
            # TODO: Remove direct Streamlit call. Raise exception or return status.
            st.error(
                f"Unsupported file type: '{file_type}'. Please upload a .txt or .docx file."
            )
            return None
        ...
        logger.exception(
            f"An unexpected error occurred while reading file: {file_name}"
        )
        # TODO: Remove direct Streamlit call. Raise exception or return status.
