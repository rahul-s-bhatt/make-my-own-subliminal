...
try:
    ...
    logging.error("Failed to import state management functions from .wizard_state. Using dummies.")
...
try:
    ...
    logging.error(f"CRITICAL: Failed to import constants from .quick_wizard_config: {e}. Defining fallbacks.")
...
try:
    ...
    logging.error(f"CRITICAL: Failed relative import of WizardAudioProcessor: {e}. Audio functions disabled.")
...
class QuickWizard:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("Retrieved TTS Generator and Audio Processor instances from cache/init.")
            ...
            if self.audio_processor is None:
                ...
                initialization_error = f"{initialization_error} {error_msg}" if initialization_error else error_msg

            # Log if any critical components failed
            if initialization_error:
                logger.critical(f"QuickWizard Initialization Failed: {initialization_error}")
    ...
    def sync_affirmation_text(self):
        ...
        st.session_state[AFFIRMATION_TEXT_KEY] = st.session_state.get("wizard_affirm_text_area", "")
        ...
        if cleared:
            logger.debug("Cleared legacy affirmation audio/sr state due to text change.")
            gc.collect()
    ...
    def sync_background_choice(self, choice_options: List[str]):
        ...
        if new_choice != current_choice:
            logger.info(f"BG choice changed: '{current_choice}'->'{new_choice}'. Updating.")
    ...
    def clear_background_upload_state(self):
        ...
        if cleared:
            logger.debug("Cleared legacy BG audio/sr/file object via file uploader callback.")
            gc.collect()
    ...
    def _get_current_processing_state(self) -> Dict[str, Any]:
        ...
        speed_state_value = st.session_state.get(AFFIRM_APPLY_SPEED_KEY, DEFAULT_APPLY_SPEED)
        logger.debug(f"DEBUG GET_STATE - Reading {AFFIRM_APPLY_SPEED_KEY}: {speed_state_value} (Type: {type(speed_state_value)})")
        state = {
            "wizard_apply_speed_change": speed_state_value,
            "wizard_speed_factor": QUICK_SUBLIMINAL_PRESET_SPEED,
            "wizard_background_choice": st.session_state.get(BG_CHOICE_KEY, DEFAULT_BG_CHOICE),
            "wizard_background_uploaded_file": st.session_state.get(BG_UPLOADED_FILE_KEY, DEFAULT_BG_UPLOADED_FILE),
            "wizard_background_noise_type": st.session_state.get(BG_NOISE_TYPE_KEY, DEFAULT_NOISE_TYPE),
            "wizard_frequency_choice": st.session_state.get(FREQ_CHOICE_KEY, DEFAULT_FREQ_CHOICE),
            "wizard_frequency_params": st.session_state.get(FREQ_PARAMS_KEY, DEFAULT_FREQ_PARAMS.copy()),
            "wizard_affirmation_volume": st.session_state.get(AFFIRMATION_VOLUME_KEY, DEFAULT_AFFIRMATION_VOLUME),
            "wizard_background_volume": st.session_state.get(BG_VOLUME_KEY, DEFAULT_BG_VOLUME),
            "wizard_frequency_volume": st.session_state.get(FREQ_VOLUME_KEY, DEFAULT_FREQ_VOLUME),
            "wizard_export_format": st.session_state.get(EXPORT_FORMAT_KEY, DEFAULT_EXPORT_FORMAT),
        }
    ...
    def _generate_affirmation_audio(self) -> Optional[AudioTuple]:
        ...
        if not self.tts_generator:
            logger.error("Cannot generate affirmation audio: TTS Engine not initialized.")
        ...
        try:
            ...
            logger.info(f"Affirmation audio generated ({len(audio_data) / sample_rate:.2f}s) in {duration:.2f}s.")
            if not isinstance(audio_data, np.ndarray) or audio_data.size == 0:
                logger.error("TTS generation resulted in empty audio data.")
                return None
            if sample_rate != GLOBAL_SR:
                logger.error(f"TTS generated audio with incorrect sample rate ({sample_rate} Hz vs expected {GLOBAL_SR} Hz).")
                return None
    ...
    def _generate_or_load_background_audio(self, bg_choice: str, bg_noise_type: str, uploaded_file: Optional[Any], target_duration_hint: float) -> Optional[AudioTuple]:
        """Loads uploaded background audio or generates noise."""
        if bg_choice == "none":
            return None
        if not self.audio_processor:
            logger.error("Cannot process background audio: Audio processor unavailable.")
            return None
        ...
        try:
            if bg_choice == "upload":
                if uploaded_file:
                    logger.info(f"Loading uploaded background file: {uploaded_file.name}")
                ...
                logger.info(f"Generating background noise: Type='{bg_noise_type}', Duration Hint={target_duration_hint:.2f}s")
                bg_tuple = self.audio_processor.generate_noise_audio(bg_noise_type, target_duration_hint, GLOBAL_SR)

            if bg_tuple and (not isinstance(bg_tuple[0], np.ndarray) or bg_tuple[0].size == 0 or bg_tuple[1] != GLOBAL_SR):
                logger.error(f"Dynamic background processing returned invalid result for '{bg_choice}'.")
                bg_tuple = None
            elif bg_tuple:
                logger.info(f"Background audio ('{bg_choice}') processed successfully.")
        except Exception as e:
            logger.exception(f"Error processing dynamic background audio (Choice: {bg_choice}).")
            bg_tuple = None
        return bg_tuple

    def _generate_frequency_audio(self, freq_choice: str, freq_params: Dict[str, Any], target_duration_hint: float) -> Optional[AudioTuple]:
        """Generates frequency-based audio."""
        if freq_choice == "None":
            return None
        if not self.audio_processor:
            logger.error("Cannot generate frequency audio: Audio processor unavailable.")
            return None

        freq_tuple: Optional[AudioTuple] = None
        try:
            logger.info(f"Generating frequency audio: Type='{freq_choice}', Params={freq_params}, Duration Hint={target_duration_hint:.2f}s")
            freq_tuple = self.audio_processor.generate_frequency_audio(freq_choice, freq_params, target_duration_hint, GLOBAL_SR)

            if freq_tuple and (not isinstance(freq_tuple[0], np.ndarray) or freq_tuple[0].size == 0 or freq_tuple[1] != GLOBAL_SR):
                logger.error(f"Dynamic frequency processing returned invalid result for '{freq_choice}'.")
                freq_tuple = None
            elif freq_tuple:
                logger.info(f"Frequency audio ('{freq_choice}') generated successfully.")
        except Exception as e:
            logger.exception(f"Error processing dynamic frequency audio (Choice: {freq_choice}).")
            freq_tuple = None
        return freq_tuple
    ...
    def generate_preview(self, duration_seconds: int = 10):
        ...
        """
        logger.info(f"QuickWizard: Starting actual preview generation ({duration_seconds}s).")
        ...
        try:
            ...
            if affirmation_tuple is None:
                st.session_state[PREVIEW_ERROR_KEY] = "Failed to generate affirmation audio for preview."
            ...
            logger.info(f"Affirmation duration (for preview hint): {affirmation_duration:.2f}s")
            ...
            if preview_mix_result is not None:
                ...
                if preview_audio is not None and preview_sr is not None:
                    ...
                    preview_buffer = save_audio_to_bytesio(preview_audio, preview_sr)  # WAV default
                    if preview_buffer and preview_buffer.getbuffer().nbytes > 0:
                        ...
                        st.session_state[PREVIEW_ERROR_KEY] = "Preview generation failed (could not save to buffer)."
                        logger.error(st.session_state[PREVIEW_ERROR_KEY])
                        st.session_state.pop(PREVIEW_BUFFER_KEY, None)
                else:
                    st.session_state[PREVIEW_ERROR_KEY] = "Preview mixing failed (processor returned invalid audio/sr)."
                    logger.error(st.session_state[PREVIEW_ERROR_KEY])
            else:
                st.session_state[PREVIEW_ERROR_KEY] = "Preview mixing failed (processor returned None)."
                logger.error(st.session_state[PREVIEW_ERROR_KEY])
            ...
            st.session_state.pop(PREVIEW_BUFFER_KEY, None)  # Ensure buffer is cleared on error
            ...
            if "preview_buffer" in locals() and preview_buffer is not None and id(preview_buffer) != id(st.session_state.get(PREVIEW_BUFFER_KEY)):
                del preview_buffer
            gc.collect()
            logger.debug("Cleaned up temporary audio tuples after preview generation attempt.")
    ...
    def process_and_export_audio(self):
        ...
        try:
            ...
            affirmation_tuple = self._generate_affirmation_audio()
            if affirmation_tuple is None:
                st.session_state[EXPORT_ERROR_KEY] = "Failed to generate affirmation audio for export."
            ...
            export_buffer, error_message = self.audio_processor.process_and_export(affirmation_tuple, background_tuple, frequency_tuple, processing_state)
            ...
            if export_buffer and not error_message:
                ...
                st.session_state[EXPORT_ERROR_KEY] = error_message or "Export failed (unknown reason)."
                logger.error(f"Export failed: {st.session_state[EXPORT_ERROR_KEY]}")
                st.session_state.pop(EXPORT_BUFFER_KEY, None)

        except Exception as e:
            # Catch unexpected errors
            logger.exception("Unhandled error occurred during process_and_export_audio wrapper.")
            if not st.session_state.get(EXPORT_ERROR_KEY):  # Avoid overwriting specific errors
            ...
            st.session_state[WIZARD_PROCESSING_ACTIVE_KEY] = False
            logger.debug(f"Reset {WIZARD_PROCESSING_ACTIVE_KEY} to False (Finally Block).")
    ...
    def render_wizard(self):
        ...
        if self._initialization_error:
            logger.error(f"Wizard rendering blocked by initialization error: {self._initialization_error}")
        ...
        if preview_triggered:
            logger.info(f"Detected {WIZARD_PREVIEW_ACTIVE_KEY}=True. Triggering preview generation.")
        ...
        # (Avoids running both if flags somehow get set simultaneously)
            ...
            logger.info(f"Detected {WIZARD_PROCESSING_ACTIVE_KEY}=True. Triggering export process.")
